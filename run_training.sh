...
if [ $# -ne 3 ]; then
    ...
    echo "Ex2 ./run_training.sh 'EXP' conf/genia.json:conf/epi.json conf/cosbi-bert.json:conf/MSbert.json:conf/scibert.json"
...
for train in "${traindata[@]}"; do
	...
	for model in "${models[@]}"; do
		...
		aws s3 cp ~/beesl/logs/${1}/${tstamp}/results_${name}_${tstamp}.txt s3://bucket
		aws s3 cp ~/beesl/logs/${1}/${tstamp}/metrics_${name}_${tstamp}.txt s3://bucket
	done
done
