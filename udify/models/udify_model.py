...
import udify.dataset_readers.universal_dependencies as reader
...
class UdifyModel(Model):
    ...
    def __init__(self,
                 ...
                 bert_path: str = "config/archive/bert-base-multilingual-cased/vocab.txt",
    ...
    def forward(self,
        ...
        for i in range(len(self.tasks)):
            ...
            if self.transformers[i] == "dependency":
            ...
            if task in gold_tags or self.transformers[i] == "dependency" and "head_tags" in gold_tags:
        ...
        if metadata is not None:
            ...
            #R: Warning, hacky!, allennlp requires them to be in the length of metadata, in the dump_lines I just use the first
            output_dict['tasks'] = [self.tasks for x in metadata]
            output_dict["transformers"] = [self.transformers for x in metadata]
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        ##TODO fix hardcoded (rependency)
        #metrics_to_track = {"lemma", "pos", "semtag", "wordnet", "supertag", "verbnet"}
        #metrics_to_track = {"bio", "ent"}#, "xpos", "feats", "lemmas", "LAS", "UAS"}
        metrics_to_track = set()
        for task, transformer in zip(self.tasks, self.transformers):
            metrics_to_track.add(task if transformer != 'dependency' else 'LAS')
